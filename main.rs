...
fn print_matches(lines: &Vec<String>, 
                 line_nums: &Vec<i32>, count: i16) -> std::io::Result<()> {
   
    // TODO Should be redundant will fix later
    let mut current_line: usize = 0;
    println!("Pattern found on {} line(s)", count);
    for line in lines {
        println!("Line {:?}: {:?}", line_nums[current_line], line);
        current_line += 1;
    }
    Ok(())
}
...
fn main() -> std::io::Result<()> {
    ...
    let _ = print_matches(&matches, &lines, count);
    Ok(())
}
